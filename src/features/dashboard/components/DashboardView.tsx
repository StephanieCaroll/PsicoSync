...
export default function DashboardView({ onLogout, userId }: DashboardViewProps) {
  ...
  const [financialData] = useState<FinancialSummary>({
    totalMonth: 5250,
    received: 4200,
    pending: 1050,
    nextReceipts: [
  ...
  const handleAddClinicalNote = () => {
    if (selectedPatient && currentNote) {
      ...
      setNotifications(prev => [`Evolução de ${selectedPatient.name} registrada`, ...prev].slice(0, 5));
    }
  };
    ...
    <div className="dash-container">
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .trend-up { color: #2E9E5B; font-size: 11px; font-weight: 500; display: flex; align-items: center; gap: 4px; }
        .trend-down { color: #C45A35; font-size: 11px; font-weight: 500; display: flex; align-items: center; gap: 4px; }
        .trend-neutral { color: #9A7040; font-size: 11px; font-weight: 500; display: flex; align-items: center; gap: 4px; }
        ...
        .mood-selector { display: flex; gap: 12px; margin-top: 8px; flex-wrap: wrap; }
      ...
      <div className="dash-notification">
        {notifications.map((notif, idx) => (
          <div key={idx} className="dash-notification-item">{notif}</div>
        ))}
      </div>
      ...
      {showPatientModal && selectedPatient && (
        <div className="dash-modal-overlay" onClick={() => setShowPatientModal(false)}>
          <div className="dash-modal" onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: "'Lora', serif", fontSize: 24, color: '#2D1F0A', marginBottom: 20 }}>
              Prontuário de {selectedPatient.name}
            </h3>
            <div className="form-group">
              <label>Informações Pessoais</label>
              <div style={{ background: '#FAF6EE', padding: 16, borderRadius: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
                <p><strong>Telefone:</strong> {selectedPatient.phone}</p>
                <p><strong>Email:</strong> {selectedPatient.email}</p>
                <p><strong>Tipo de Terapia:</strong> {selectedPatient.therapyType}</p>
                <p><strong>Última Sessão:</strong> {selectedPatient.lastSession ? new Date(selectedPatient.lastSession).toLocaleDateString('pt-BR') : '—'}</p>
              </div>
            </div>
            <div className="form-group">
              ...
              <div style={{ maxHeight: 240, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
                {clinicalNotes.filter(n => n.patientId === selectedPatient.id).length === 0 && (
                  <p style={{ fontSize: 13, color: '#9A7040' }}>Nenhuma evolução registrada.</p>
                )}
                {clinicalNotes.filter(n => n.patientId === selectedPatient.id).map(note => (
                  <div key={note.id} style={{ padding: 12, background: '#FAF6EE', borderRadius: 8 }}>
      ...
      {showNoteModal && selectedPatient && (
        <div className="dash-modal-overlay" onClick={() => setShowNoteModal(false)}>
          <div className="dash-modal" onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: "'Lora', serif", fontSize: 24, color: '#2D1F0A', marginBottom: 20 }}>
              Registrar Evolução — {selectedPatient.name}
            </h3>
            <div className="form-group">
        ...
        <div className="dash-profile">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ overflow: 'hidden' }}>
              <div style={{ fontSize: 13, color: 'white', fontWeight: 500 }}>Dra. Ana Beatriz</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>CRP 12/34567</div>
            </div>
          </div>
          <button
            onClick={onLogout}
            style={{ width: '100%', padding: 10, background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s', borderRadius: 6 }}
        ...
        <main className="dash-content">
              ...
              <div className="dash-kpi-grid">
                ...
                <div className="dash-card dash-anim d-3">
                  ...
                  <div className="trend-down">
                    ...
                    R$ {financialData.pending.toLocaleString()} em aberto
                  </div>
                </div>
              ...
              <div className="dash-layout-grid">
                ...
                <div className="dash-card dash-anim d-3" style={{ display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, borderBottom: '1px solid #FAF6EE', paddingBottom: 16 }}>
                    ...
                    <button onClick={() => handleTabChange('agenda')} style={{ fontSize: 11, color: '#AD6D15', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      Abrir Agenda
                    </button>
                  </div>
                  {appointments.map((apt, i) => (
                    <div key={i} className="dash-list-item">
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                        ...
                        <div style={{ width: 3, height: 36, background: apt.type === 'online' ? '#2E9E5B' : '#AD6D15', borderRadius: 2, marginTop: 2 }} />
                        <div>
                          <div style={{ fontSize: 15, fontWeight: 600, color: '#2D1F0A', marginBottom: 4 }}>{apt.patientName}</div>
                          <div style={{ fontSize: 12, color: '#9A7040' }}>
                      ...
                      <div>
                        {apt.type === 'online' ? (
                          <button className="btn-action">▶ Iniciar</button>
                        ) : (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                  <div className="dash-card dash-anim d-3">
                    <h3 style={{ fontFamily: "'Lora', serif", fontSize: 18, color: '#2D1F0A', margin: 0, marginBottom: 16, borderBottom: '1px solid #FAF6EE', paddingBottom: 16 }}>Alertas Clínicos</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {patients.filter(p => p.pendingAmount > 0).map(patient => (
                        <div key={patient.id} style={{ padding: 14, background: '#FEF0EC', borderLeft: '3px solid #C45A35', borderRadius: '0 8px 8px 0' }}>
                          <div style={{ fontSize: 12, fontWeight: 700, color: '#993C1D', marginBottom: 4, textTransform: 'uppercase' }}>Pagamento Pendente</div>
                          <div style={{ fontSize: 12, color: '#C45A35' }}>{patient.name} — R$ {patient.pendingAmount.toLocaleString()}</div>
                        </div>
                      ))}
                      <div style={{ padding: 14, background: '#F5ECD8', borderLeft: '3px solid #AD6D15', borderRadius: '0 8px 8px 0' }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: '#9A7040', marginBottom: 4, textTransform: 'uppercase' }}>Prontuários Pendentes</div>
                        <div style={{ fontSize: 12, color: '#9A7040' }}>{patients.length - clinicalNotes.length} pacientes aguardando evolução</div>
                      </div>
                    </div>
                  </div>

                  <div className="dash-card dash-anim d-3">
                    <h3 style={{ fontFamily: "'Lora', serif", fontSize: 18, color: '#2D1F0A', margin: 0, marginBottom: 16, borderBottom: '1px solid #FAF6EE', paddingBottom: 16 }}>Pacientes em Espera</h3>
                    {patients.filter(p => p.status === 'waiting').map(patient => (
                      <div key={patient.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0' }}>
                        ...
                        <button className="btn-action" onClick={() => handleTabChange('agenda')}>Agendar</button>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {patients.slice(0, 4).map(patient => (
                        <tr
                          key={patient.id}
                          style={{ borderBottom: '1px solid #FAF6EE', cursor: 'pointer' }}
          ...
          {activeTab === 'pacientes' && (
            <div className="dash-anim d-1">
              <PatientsTab
                userId={userId}
                onSelectPatient={handleSelectAppwritePatient}
              />
            </div>
          )}
            ...
            <div className="dash-anim d-1">
              <div className="dash-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                  ...
                  <button className="btn-primary" onClick={() => { if (patients[0]) { setSelectedPatient(patients[0]); setShowNoteModal(true); } }}>
                    + Nova Evolução
                  </button>
                </div>
                  ...
                  {clinicalNotes.map(note => (
                    <div key={note.id} style={{ padding: 20, background: '#FAF6EE', borderRadius: 12, border: '1px solid #E8D9BE' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                        ...
                        {note.mood && (
                          <span className="badge" style={{ background: '#F5ECD8', color: '#AD6D15' }}>Estado: {note.mood}</span>
                        )}
                      </div>
          ...
          {activeTab !== 'dashboard' && activeTab !== 'pacientes' && activeTab !== 'evolucao' && activeTab !== 'resumo-fin' && (
            <div className="dash-anim d-1" style={{ minHeight: 400, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#9A7040', textAlign: 'center', background: 'white', border: '1px dashed #D9C49A', borderRadius: 12, padding: 24 }}>
              ...
              <p style={{ fontSize: 14, maxWidth: 420, lineHeight: 1.6, color: '#5A3E20' }}>
                Este módulo está sendo desenvolvido e em breve estará disponível integrado ao Appwrite.
              </p>
